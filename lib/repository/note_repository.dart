...
class NoteRepository {
  ...
  Future<void> saveOpinion(Opinion opinion) async {
    await noteDatabase.putDynamicWithUid(opinion.uid, opinion.toJsonString());
  }

  Future<Opinion> getOpinion(String uid) async{
    return Opinion.fromJsonString(await noteDatabase.getDynamic(uid) as String);
  }

  Future<void> deleteOpinion(String uid) async {
    assert(uid.startsWith('opinion'), 'uid error when note repository delete opinion');
    return noteDatabase.deleteDynamic(uid);
  }


  Future<List<String>> getOpinionKeys() async {
    return await noteDatabase.keysStartsWith('opinion');
  }
}
...
class PreferenceInfo {
  ...
  Map<String, dynamic> toJson() {
    return <String, dynamic>{'lastOpenedNote': lastOpenedNote, 'sortRule': sortRule};
  }
